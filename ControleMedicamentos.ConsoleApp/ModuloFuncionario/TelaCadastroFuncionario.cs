...
namespace ControleMedicamentos.ConsoleApp.ModuloFuncionario
{
    public class TelaCadastroFuncionario : TelaBase, ITelaCadastravel
    {
        ...
        public void VerificarDisponibilidadeRemedio(int quantidadeRequisitada, Medicamento medicamento)
        {
            if(quantidadeRequisitada > medicamento.Quantidade)
            {
                _notificador.ApresentarMensagem($"Não é possível retirar {quantidadeRequisitada} pois há somente {medicamento.Quantidade}, tente novamente...", TipoMensagem.Erro);
            }
            else if(quantidadeRequisitada == medicamento.Quantidade)
            {
                _notificador.ApresentarMensagem($"Foram retirados {quantidadeRequisitada}, porém, agora o estoque está vazio", TipoMensagem.Atencao);
            }
            else
            {
                medicamento.AlterarQuantidade(quantidadeRequisitada);
                _notificador.ApresentarMensagem($"Foram retirados {quantidadeRequisitada}\nEstoque atual {medicamento.Quantidade}", TipoMensagem.Sucesso);
            }
        }
